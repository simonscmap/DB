...
BEGIN
	...
	SET @selList =  'YEAR([time]) [year], DATEPART(WEEK, [time]) [week], AVG(lat) AS lat, AVG(lon) AS lon, AVG(' + RTRIM(LTRIM(@field)) +') AS ' + RTRIM(LTRIM(@field)) + ', STDEV(' + RTRIM(LTRIM(@field)) + ') AS ' + RTRIM(LTRIM(@field)) + '_std '
	...
	BEGIN
		SET @selList =  'YEAR([time]) [year], DATEPART(WEEK, [time]) [week], AVG(lat) AS lat, AVG(lon) AS lon, AVG(depth) AS depth, AVG(' + RTRIM(LTRIM(@field)) +') AS ' + RTRIM(LTRIM(@field)) + ', STDEV(' + RTRIM(LTRIM(@field)) + ') AS ' + RTRIM(LTRIM(@field)) + '_std '
		SET @query = 'SELECT ' + @selList + ' FROM ' + RTRIM(LTRIM(@tableName)) + 
		@timeQuery +
		@latQuery +
		@lonQuery +
