...
CREATE PROC [dbo].[uspftleBkg] @ftleTable NVARCHAR(MAX), @ftleField NVARCHAR(MAX), @ftleValue NVARCHAR(MAX), @bkgTable NVARCHAR(MAX), @bkgField NVARCHAR(MAX), @dt1 NVARCHAR(MAX), @dt2 NVARCHAR(MAX), @lat1 NVARCHAR(MAX), @lat2 NVARCHAR(MAX), @lon1 NVARCHAR(MAX), @lon2 NVARCHAR(MAX), @margin NVARCHAR(MAX), @extV NVARCHAR(MAX), @extVV NVARCHAR(MAX)
AS
BEGIN
	...
	'SELECT [#tblBkg].[time], [#tblBkg].lat, [#tblBkg].lon, [#tblBkg].'+ RTRIM(LTRIM(@bkgField)) +' FROM #tblBkg WHERE NOT EXISTS ' + 
