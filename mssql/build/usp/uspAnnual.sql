...
CREATE PROC [dbo].[uspAnnual] @tableName NVARCHAR(MAX), @field NVARCHAR(MAX), 
...
BEGIN
	...
	SET @selList =  'YEAR([time]) [year], AVG(lat) AS lat, AVG(lon) AS lon, AVG(' + RTRIM(LTRIM(@field)) +') AS ' + RTRIM(LTRIM(@field)) + ', STDEV(' + RTRIM(LTRIM(@field)) + ') AS ' + RTRIM(LTRIM(@field)) + '_std '

	DECLARE @groupOrder AS NVARCHAR(MAX);
	SET @groupOrder = ' GROUP BY YEAR([time]) ORDER BY [year]'	
	...
	BEGIN
		SET @selList =  'YEAR([time]) [year], AVG(lat) AS lat, AVG(lon) AS lon, AVG(depth) AS depth, AVG(' + RTRIM(LTRIM(@field)) +') AS ' + RTRIM(LTRIM(@field)) + ', STDEV(' + RTRIM(LTRIM(@field)) + ') AS ' + RTRIM(LTRIM(@field)) + '_std '
		SET @query = 'SELECT ' + @selList + ' FROM ' + RTRIM(LTRIM(@tableName)) + 
		@timeQuery +
		@latQuery +
		@lonQuery +
