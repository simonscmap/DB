...
BEGIN
	...
	SET @selList =  'YEAR([time]) [year], AVG(lat) AS lat, AVG(lon) AS lon, AVG(' + @field +') AS ' + @field + ', STDEV(' + @field + ') AS ' + @field_std
	...
	BEGIN
		SET @selList =  'YEAR([time]) [year], AVG(lat) AS lat, AVG(lon) AS lon, AVG(depth) AS depth, AVG(' + @field +') AS ' + @field + ', STDEV(' + @field + ') AS ' + @field_std
		SET @query = 'SELECT ' + @selList + ' FROM ' + @tableName + 
		@timeQuery +
		@latQuery +
		@lonQuery +
